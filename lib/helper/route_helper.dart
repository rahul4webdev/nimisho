...
import 'package:sixam_mart/features/subscription/add_money_screen.dart';
import 'package:sixam_mart/features/subscription/edit_subscription.dart';
import 'package:sixam_mart/features/subscription/my_subscriprion.dart';
...
import '../features/subscription/item_subscription_screen.dart';
import '../features/subscription/subscription_checkout_screen.dart';
...
class RouteHelper {
  ...
  static const String itemSubscription = '/item-subscription';
  static const String endiSubscription = '/edit-subscription';
  ...
  static String getSubscriptionRoute() => subscription;
  ...
  static String getPaymentRoute(String id, int? user, String? type, double amount, bool? codDelivery, String? paymentMethod, {required String guestId, String? contactNumber, String? addFundUrl, String? subscriptionUrl, int? storeId, bool? createAccount, int? createUserId, bool? istypeSubscription}
      ) => '$payment?id=$id&user=$user&type=$type&amount=$amount&cod-delivery=$codDelivery&add-fund-url=$addFundUrl&payment-method=$paymentMethod&guest-id=$guestId&number=$contactNumber&subscription-url=$subscriptionUrl&store_id=$storeId&create_account=$createAccount&create_user_id=$createUserId&isTypeSubscription=$istypeSubscription';
  static String getCheckoutRoute(String page,{int? storeId}) => '$checkout?page=$page&store-id=$storeId';
  static String getSubscriptionCheckoutRoute(String page,{int? storeId}) => '$subscriptioncheckout?page=$page&store-id=$storeId';
  ...
  static String getEditSubscriptionRoute(int index) => '$endiSubscription?index=$index';
  static String getItemDetailsRoute(int? itemID, bool isRestaurant) => '$itemDetails?id=$itemID&page=${isRestaurant ? 'restaurant' : 'item'}';
  static String getItemSubscriptionRoute(int? itemID, bool isRestaurant) => '$itemSubscription?id=$itemID&page=${isRestaurant ? 'restaurant' : 'item'}';
  ...
  static String getAddMoneyRoute({String? subscriptionId,String? paymentMethode}) => '$addMoney?subscriptionId=$subscriptionId&payment_method=$paymentMethode';
  ...
  static List<GetPage> routes = [
    ...
    GetPage(name: subscription, page: () => getRoute(const MySubscriprion())),
    ...
    GetPage(name: payment, page: () {
      ...
      bool isTypeSubscription = Get.parameters['isTypeSubscription'] == 'true';
      ...
      return getRoute(AppConstants.payInWevView ? PaymentWebViewScreen(
        ...
        contactNumber: number, subscriptionUrl: subscriptionUrl, storeId: storeId, createAccount: createAccount,istypeSubscription:isTypeSubscription
      ) : PaymentScreen(
        orderModel: order, isCashOnDelivery: isCodActive, addFundUrl: addFundUrl, paymentMethod: paymentMethod, guestId: guestId,
        contactNumber: number, subscriptionUrl: subscriptionUrl, storeId: storeId, createAccount: createAccount, createUserId: createUserId,isTypeSubscription:isTypeSubscription
      ));
    }),
    ...
    GetPage(name: subscriptioncheckout, page: () {
      SubscriptionCheckoutScreen? checkoutScreen = Get.arguments;
      // bool fromCart = Get.parameters['page'] == 'cart';
      return getRoute(checkoutScreen ?? (/*!fromCart ? const NotFound() :*/ SubscriptionCheckoutScreen(
        cartList: null, fromCart: Get.parameters['page'] == 'cart', storeId: Get.parameters['store-id'] != 'null' ? int.parse(Get.parameters['store-id']!) : null,
      )));
    }),
    ...
    GetPage(name: endiSubscription, page: () => getRoute(Get.arguments ??  EditMySubscriprion(index: int.parse(Get.parameters['index']!)))),
    ...
    GetPage(name: itemSubscription, page: () => getRoute(Get.arguments ?? ItemSubscriptionScreen(itemId: int.parse(Get.parameters['id']!), inStorePage: Get.parameters['page'] == 'restaurant'))),
    ...
    GetPage(name: addMoney, page: () {
      return getRoute(Get.arguments ?? AddMoneyScreen(orderId: int.parse(Get.parameters['subscriptionId']!), paymentMethode: Get.parameters['payment_method'] ?? "razor_pay"));
    }),
