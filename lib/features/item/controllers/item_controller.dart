...
import 'package:sixam_mart/features/subscription/item_subscription_screen.dart';
...
class ItemController extends GetxController implements GetxService {
  ...
  String _selectedSubscription = 'Weekly';
  ...
  TimeOfDay _endTime = TimeOfDay(hour: 6, minute: 0);
  TimeOfDay get endTime => _endTime;
  int _subscriptionqauntity = 0;
  int get subscriptionqauntity => _subscriptionqauntity;
  ...
  void setSubscription({required String selectedSubType}) {
    _selectedSubscription = selectedSubType;
    update();
  }

  void setSubscriptionQuantity({required int quantity}) {
    _subscriptionqauntity = quantity;
    update();
  }


  void subscriptionqauntityUpdate({required bool type}) {

    if(type){
      _subscriptionqauntity++;
    }else{
      if(subscriptionqauntity !=1){
        _subscriptionqauntity--;
      }
    }
    update();
  ...
  void setstartDate({required DateTime selectStartDate}) {
    _startDate = selectStartDate;
    update();
  }

  void setendDate({required DateTime selectEndTime}) {
    if (_startDate == null) {
      Get.showSnackbar(GetSnackBar(
        title: "Invalid Date!",
        message: "Please Select After Start Date!",
        backgroundColor: Colors.red,
        borderRadius: 10,
        margin: EdgeInsets.all(10),
        duration: Duration(milliseconds: 1500), // Auto remove after 1.5s
      ));
    }

    if (selectEndTime.isBefore(_startDate!)) {
      Get.showSnackbar(GetSnackBar(
        title: "Invalid Date!",
        message: "Please Select After Start Date!",
        backgroundColor: Colors.red,
      ...
      _endDate = selectEndTime;
      update();
    }
  }

  void setSheduleTime(
      {required TimeOfDay selectStartTime, required TimeOfDay selectEndTime}) {

    if (selectStartTime.isBefore(selectEndTime) ||
        (selectStartTime.hour == selectEndTime.hour &&
            selectStartTime.minute == selectEndTime.minute)) {
      _endTime = selectEndTime;
      _startTime = selectStartTime;
      update();
    } else {
      Get.showSnackbar(GetSnackBar(
        title: "Invalid Shedule Time!",
        message: "Please Select Start Time Greater Than End Time!" ,
        backgroundColor: Colors.red,
        borderRadius: 10,
        margin: EdgeInsets.all(10),
        duration: Duration(milliseconds: 1500),
  ...
  void setSelectedDays({required int index}) {
    bool isSelected = selectedDays.contains(weekDaysFull[index]);
    if (isSelected) {
      _selectedDays.remove(weekDaysFull[index]);
    } else {
      _selectedDays.add(weekDaysFull[index]);
    }
    update();
  }


  void setEditSelectedDays({required List<String> selectedDayss}) {
    _selectedDays.clear();
    _selectedDays.addAll(selectedDayss ?? [] );
    update();
  }
  void setSubscriptionData() {
    _subscriptionData!.type = _selectedSubscription;
    _subscriptionData!.startDate = "${_startDate}";
    _subscriptionData!.endDate = "${_endDate}";
    _subscriptionData!.scheduleTime = "${_startTime}-${_endTime}";
    _subscriptionData!.selectedDays = _selectedDays;
    update();
  }


  Future<void> updateYourSubscription(int? id) async {

    _isLoading = true;
    Map<String, String> map = {
      "id": '${id ?? 0}',
      "type":_selectedSubscription,
      "start_date":"${_startDate}",
      "end_date":"${_endDate}",
      "shedual_time":"${_startTime}-${_endTime}",
      "week_day":selectedDays.toString(),
      "quantity":'${subscriptionqauntity}' ,
    };
    Response response;
    response = await itemServiceInterface.updateMySubscription(map);
  ...
  void navigateToSubscriptionPage(Item? item, BuildContext context,
      {bool inStore = false, bool isCampaign = false}) {
    Get.toNamed(RouteHelper.getItemSubscriptionRoute(item?.id, inStore),
        arguments: ItemSubscriptionScreen(
            itemId: item!.id ?? 0,
            inStorePage: inStore,
            isCampaign: isCampaign));
  }
