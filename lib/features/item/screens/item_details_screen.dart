...
class ItemDetailsScreen extends StatefulWidget {
  ...
  const ItemDetailsScreen({super.key, required this.itemId, required this.inStorePage, this.isCampaign});
          ...
          body: SafeArea(child: (item != null) ? ResponsiveHelper.isDesktop(context) ? DetailsWebViewWidget(
            ...
            Expanded(child: SingleChildScrollView(
              ...
              child: Center(child: SizedBox(width: Dimensions.webMaxWidth, child: Column(
                  ...
                  children: [
                    ItemImageViewWidget(item: item, isCampaign: widget.isCampaign ?? false),
                    ...
                    children: [
                      ...
                      Expanded(
                        child: CustomButton(
                          ...
                          onPressed: (
                           !Get.find<SplashController>().configModel!.moduleConfig!.module!.stock! || stock! > 0) ?  () async {
                            Get.find<CartController>().clearCartOnline().then((success) async {
                              if(success) {
                                await cartController.addToCartOnline(cart!).then((success) async {
                                  if(success){
                                    itemController.setExistInCart(item, null);
                                    _key.currentState!.shake();
                                   isLoggedIn ? Get.offNamed(RouteHelper.getSubscriptionCheckoutRoute('cart'))
                                   :await Get.toNamed(RouteHelper.getSignInRoute(Get.currentRoute));
                                    if(isLoggedIn) {
                                      Get.offNamed(RouteHelper.getSubscriptionCheckoutRoute('cart'));
                                    }
