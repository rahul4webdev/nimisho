...
class CheckoutController extends GetxController implements GetxService {
  ...
  String? _payment_order_id;
  String? get payment_order_id => _payment_order_id;
  ...
  SurgePriceModel? get surgePrice => _surgePrice;
   ...
   Map<String, dynamic>? get weekData => _weekData;
  ...
  Future<void> getData() async {
   Response response = await checkoutServiceInterface.getweeklySub();
   print(response.body);
   print('farukh----');
   _weekData = response.body;
   update();
  }
  ...
  Future<String> placeSubscriptionOrder(PlaceOrderBodyModel placeOrderBody, int? zoneID, double amount, double? maximumCodOrderAmount, bool fromCart, bool isCashOnDeliveryActive, List<XFile>? orderAttachment, {bool isOfflinePay = false}) async {
    List<MultipartBody>? multiParts = [];
    for(XFile file in orderAttachment!) {
      multiParts.add(MultipartBody('order_attachment[]', file));
    }
    ...
    String orderID = '';
    String userID = '';
    Response response = await checkoutServiceInterface.placeSubscriptionOrder(placeOrderBody, multiParts);
    _isLoading = false;
    if (response.statusCode == 200) {
      Get.offNamed(RouteHelper.getAddMoneyRoute(subscriptionId: response.body['order_id'].toString(),paymentMethode:'$digitalPaymentName' ));
    }
    update();
      ...
      if(paymentMethodIndex == 2) {
        if(GetPlatform.isWeb) {
          ...
          selectedUrl = '${AppConstants.baseUrl}/payment-mobile?order_id=$orderID&&customer_id=${Get.find<ProfileController>().userInfoModel?.id ?? (userID.isNotEmpty ? userID : AuthHelper.getGuestId())}'
              '&payment_method=$digitalPaymentName&payment_platform=web&&callback=$protocol//$hostname${RouteHelper.orderSuccess}?id=$orderID&type=$isSubscription&&status=';

          html.window.open(selectedUrl,"_self");
        } else{

          if(digitalPaymentName == "razor_pay"){
            String selectedUrl;
            _payment_order_id = orderID;
            selectedUrl = '/payment-mobile?order_id=$orderID&customer_id=${Get.find<ProfileController>().userInfoModel?.id ?? (userID.isNotEmpty ? userID : AuthHelper.getGuestId())}'
                '&payment_method=$digitalPaymentName';
            Response response = await checkoutServiceInterface.getOrderPaymentId(selectedUrl);
            Response razorPayOrder = await checkoutServiceInterface.createRazorpayOrder(amount);
            _payment_id = response.body['payment_id'];
            ...
            Get.offNamed(RouteHelper.getPaymentRoute(
                ...
                contactNumber: contactNumber,istypeSubscription: isSubscription
            ));
          }
  ...
  void handlePaymentSuccessResponse(PaymentSuccessResponse response) async{
    Response responses = await checkoutServiceInterface.getPaymentVerify('$payment_id', '${response.paymentId}','${response.orderId}','${response.signature}');
    Get.back();
    Get.offNamed(RouteHelper.getOrderSuccessRoute('${payment_order_id}', "", createAccount: _isCreateAccount));
