...
class SubscriptionCheckoutScreen extends StatefulWidget {
  ...
  final bool fromCart;
  final int? storeId;
  const SubscriptionCheckoutScreen({super.key, required this.fromCart, required this.cartList, required this.storeId});

  @override
  SubscriptionCheckoutScreenState createState() => SubscriptionCheckoutScreenState();
}

class SubscriptionCheckoutScreenState extends State<SubscriptionCheckoutScreen> {
                ...
                child: FooterView(child: SizedBox(
                  ...
                  child: ResponsiveHelper.isDesktop(context) ? Padding(
                    ...
                    TopSection(
                      ...
                      addressList: addressList,
                      tomorrowClosed: tomorrowClosed, todayClosed: todayClosed, module : module, price: price,
                      discount: discount, addOns: addOns, address: address, cartList: _cartList, isCashOnDeliveryActive: false,
                      isDigitalPaymentActive: _isDigitalPaymentActive!, isWalletActive: _isWalletActive, storeId: widget.storeId,
                      total: total, isOfflinePaymentActive: _isOfflinePaymentActive, guestNameTextEditingController: guestContactPersonNameController,
                      guestNumberTextEditingController: guestContactPersonNumberController, guestNumberNode: guestNumberNode,
                      guestEmailController: guestEmailController, guestEmailNode: guestEmailNode,
                ...
                child: Column(
                  children: [
                    Padding(
                      ...
                      child: Row(children: [
                        Text(
                          checkoutController.isPartialPay ? 'due_payment'.tr : 'Per Order Amount'.tr,
    ...
    return Container(
      ...
      child: SafeArea(
        child: CustomButton(
          ...
          Get.find<ItemController>().setSubscriptionData();
          ...
          if(isGuestLogIn && checkoutController.guestAddress == null && checkoutController.orderType != 'take_away') {
            ...
            if(ResponsiveHelper.isDesktop(context)){
              Get.dialog(Dialog(backgroundColor: Colors.transparent, child: PaymentMethodBottomSheet(shudule: true,isCashOnDeliveryActive: false, isDigitalPaymentActive: _isDigitalPaymentActive!,
              ...
              showModalBottomSheet(
                ...
                builder: (con) => PaymentMethodBottomSheet(shudule: true,isCashOnDeliveryActive: false, isDigitalPaymentActive: _isDigitalPaymentActive!,
                  isWalletActive: _isWalletActive, storeId: widget.storeId, totalPrice: total, isOfflinePaymentActive: _isOfflinePaymentActive,
                ),
              );
            }
          ...
          }else if (Get.find<ItemController>().subscriptionData?.startDate == null
              || Get.find<ItemController>().subscriptionData?.startDate == 'null'
              || Get.find<ItemController>().subscriptionData?.endDate == null
              || Get.find<ItemController>().subscriptionData?.endDate == 'null') {

            showCustomSnackBar('Please select subscription start and end date');
          }
          else {
            ...
            if(widget.storeId == null){
              ...
              for (int index = 0; index < _cartList!.length; index++) {
                ...
                carts.add(OnlineCart(
                  ...
                  Get.find<SplashController>().getModuleConfig(cart.item!.moduleType).newVariation! ? null : cart.variation,
                  Get.find<SplashController>().getModuleConfig(cart.item!.moduleType).newVariation! ? variations : null,
                  cart.quantity, addOnIdList, cart.addOns, addOnQtyList, 'Item', itemType: "App\\Models\\Item",
                ));
              }
              ...
              PlaceOrderBodyModel placeOrderBody = PlaceOrderBodyModel(
                subscriptionData: Get.find<ItemController>().subscriptionData,
                cart: carts, couponDiscountAmount: Get.find<CouponController>().discount, distance: checkoutController.distance,
                ...
                extraPackagingAmount: Get.find<CartController>().needExtraPackage ? checkoutController.store!.extraPackagingAmount : 0,
                createNewUser: checkoutController.isCreateAccount ? 1 : 0, password: guestPasswordController.text,isSubscriptionOrder: true
              );
              ...
              if(checkoutController.paymentMethodIndex == 3){
                ...
                checkoutController.placeSubscriptionOrder(placeOrderBody, checkoutController.store!.zoneId, total, maxCodOrderAmount, widget.fromCart, _isCashOnDeliveryActive!, checkoutController.pickedPrescriptions);
              }
            }else{
