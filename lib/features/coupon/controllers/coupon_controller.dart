...
class CouponController extends GetxController implements GetxService {
  ...
  Future<void> pauseYourSubscription(int? id) async {
    Response response;

    print("zain----1");
    response = await couponServiceInterface.pauseYourSubscription(id);
  ...
  Future<void> getSubscriptionList() async {
    List<Subscription>? couponList = await couponServiceInterface.getSubscriptionList();
    if (couponList != null) {
      _subscriptionList = [];
      _subscriptionList!.addAll(couponList);
    }
