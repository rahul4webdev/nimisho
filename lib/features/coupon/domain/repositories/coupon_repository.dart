...
class CouponRepository implements CouponRepositoryInterface {
  ...
  Future<Response> pauseYourSubscription(int? subscription_id) async {
    ...
    Response response = await apiClient.postData(AppConstants.pauseSubscription,{"id": '$subscription_id'} );
    print(response.body);
    return response;
  }

  Future<List<Subscription>?> getSubscriptionList() async {
    List<Subscription>? couponList;
    Response response = await apiClient.getData(AppConstants.subscriptionListUri);
    ...
    if (response.statusCode == 200) {
      couponList = [];
      response.body.forEach((category) {
        Subscription coupon = Subscription.fromJson(category);
        couponList!.add(coupon);
      });
    }
    return couponList;
