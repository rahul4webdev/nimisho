...
class PaymentWebViewScreen extends StatefulWidget {
  ...
  const PaymentWebViewScreen({super.key, required this.orderModel, required this.isCashOnDelivery, this.addFundUrl, required this.paymentMethod,
    required this.guestId, required this.contactNumber, this.subscriptionUrl, this.storeId, this.createAccount = false,this.istypeSubscription =false});
...
class PaymentScreenState extends State<PaymentWebViewScreen> {
  ...
  void initState() {
    super.initState();
    print("farukh-------");
    print("${widget.istypeSubscription}");
    print("farukh-------");

    if(widget.addFundUrl == '' && widget.addFundUrl!.isEmpty && widget.subscriptionUrl == '' && widget.subscriptionUrl!.isEmpty){
      selectedUrl = '${AppConstants.baseUrl}/payment-mobile?customer_id=${widget.orderModel.userId == 0 ? widget.guestId : widget.orderModel.userId}&order_id=${widget.orderModel.id}&payment_method=${widget.paymentMethod}&istypeSubscription=${widget.istypeSubscription}';
      print("farukh-------1");
